 	...
 	public function valida_site($sup){
 		...
 		foreach ($this->array_supervisor as $super) {
 			if($sup['nome'] == $super->get('nome'))
 			{
 				...
 				if($site == 'MCZ'){
 					$this->mcz++;
 					$this->fat_mcz = $this->fat_mcz + $sup['tkm_base'];
 				}
 				elseif ($site == 'NVA') {
 					$this->nva++;
 					$this->fat_nva = $this->fat_nva + $sup['tkm_base'];
 				}
 				else
 				{
 					$this->error++;
 	...
 	public function atualizar_dados()
 	{
 		...
 		if (($handle = fopen("../dados.csv", "r")) !== FALSE) {
 			...
 			for ($i = 0; $i < count($info_venda); $i++){
 				//var_dump($info_venda[$i][19]);

	    	//extrair o primeiro nome do supervisor
 				$temp_sup = explode(" ",substr($info_venda[$i][8],4));

 				$supervisor[$i] = array('nome'=>$temp_sup[0],'tkm_base'=>$info_venda[$i][19]);

 				//var_dump($supervisor[$i]);
...
public function main() {
	...
	$last_hour = date("H:i:s",strtotime($objConexao->buscar_hora()));
	//recupera o dia passado
	$last_day = date("d",strtotime($objConexao->buscar_hora()));
	//recupera o dia atual
	$today = date("d");	
	//recupera a hora atual
	$hora_atual = date('H:i:s');
	//armazena as horas em um array para conversão em segundos
	$list = array($last_hour,$hora_atual);

	$itera_list = 0;

	//força a atualização
	if($today > $last_day) //isset($_REQUEST['up']) && $_REQUEST['up'] == 't')
	{
		...
		$objConexao->atualizar_cache($this->nva,$this->mcz,$this->fat_nva, $this->fat_mcz, $this->array_supervisor);
		...
		if($hora[1] >= ($hora[0] + 121))
		{
			...
			foreach ($cache as $value) {
				...
				$this->fat_mcz = $value['MCZ_FAT'];
				$this->fat_nva = $value['NVA_FAT'];
	...
	$result = array(
		...
		'error'=>$this->error,
		'fat_mcz'=>$this->fat_mcz,
		'fat_nva'=>$this->fat_nva
	);
